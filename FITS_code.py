...
def plot_dynamic_spectrum(fits_file, output_png=None):
    ...
    with fits.open(fits_file) as hdulist:
        ...
        dat_scl = dat_scl.reshape((5,4,1280))
        dat_offs = data_table['DAT_OFFS']  # 'DAT_OFFS' is the column name
        dat_offs = dat_offs.reshape((5,4,1280))
        ...
        for subint in range(5):
            ...
            dynamic_spectrum_stokes1 = hdulist[1].data['DATA'][subint, :, 0, :]# First Stokes parameter
            dynamic_spectrum_stokes1 = dynamic_spectrum_stokes1*dat_scl + dat_offs
            dynamic_spectrum_stokes2 = hdulist[1].data['DATA'][subint, :, 1, :]  # Second Stokes parameter
            dynamic_spectrum_stokes2 = dynamic_spectrum_stokes2*dat_scl + dat_offs

            # Apply DAT_SCL and DAT_OFFS to the data
            corrected_spectrum_stokes1 = dynamic_spectrum_stokes1 #dynamic_spectrum_stokes1 * dat_scl[subint, :, np.newaxis] + dat_offs[subint, :, np.newaxis]
            corrected_spectrum_stokes2 = dynamic_spectrum_stokes2 #dynamic_spectrum_stokes2 * dat_scl[subint, :, np.newaxis] + dat_offs[subint, :, np.newaxis]

            # Sum the corrected Stokes parameters

            summed_dynamic_spectrum = corrected_spectrum_stokes1 + corrected_spectrum_stokes2
            summed_dynamic_spectrum = summed_dynamic_spectrum#* dat_scl[subint, :, np.newaxis] + dat_offs[subint, :, np.newaxis]

            print(summed_dynamic_spectrum.shape)
